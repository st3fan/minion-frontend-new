...
from flask import g, jsonify, session, request, Response
...
def accept_invite(recipient, data):
    ...
    if invite:
        invite = _backend_control_invite(
            invite_id,
...
def get_history_report(user=None):
    ...
    if user is not None:
        params['user'] = user
    r = requests.get(config['backend-api']['url'] +
                     '/reports/history', params={'user': session['email']})
    r.raise_for_status()
    j = r.json()
    if not j.get('success'):
        return None
...
def get_issues_report(user=None):
    ...
    if user is not None:
        params['user'] = user
    r = requests.get(config['backend-api']['url'] +
                     '/reports/issues', params={'user': session['email']})
    r.raise_for_status()
    j = r.json()
    if not j.get('success'):
        return None
...
def _backend_get_invite(id=None, recipient=None, sender=None):
    if id is not None:
        ...
        r = requests.get(config['backend-api']
                         ['url'] + '/invites', params=params)
    r.raise_for_status()
    j = r.json()
    if not j.get('success'):
        return None
...
def _backend_control_invite(id, action_data):
    r = requests.post(
        config['backend-api']['url'] + '/invites/%s/control' % id,
        headers={'Content-Type': 'application/json'},
...
def api_session():
    return jsonify(
        success=True,
        data={'email': session['email'], 'role': session['role']}
    )
...
def ldap_login(request):
    ...
    # Active Directory (and others) don't require a full DN to bind (such as
    # samAccountName=april)
    if 'baseDN' in config['login']['ldap']:
        auth_query = (
            '{ldap[usernameAttribute]}={username},{ldap[baseDN]}'
            .format(ldap=config['login']['ldap'], username=username)
        )
    else:
        auth_query = '{ldap[usernameAttribute]}={username}'.format(
    ...
    # Retrieve mail
    results = ld.search_s(auth_query, ldap.SCOPE_SUBTREE, attrlist=[
                          str(config['login']['ldap']['emailAttribute'])])
    try:
...
# #
# # WARNING WARNING WARNING - Everything below is temporary and will likely
# # not survive the next iteration of Minion where the focus will be more on
# # the backend side of things. Don't assume
        ...
        for s in j['scan']['sessions']:
            for issue in s['issues']:
                if issue['Id'] == minion_issue_id:
                    SESSION_FIELDS = ('plugin', 'artifacts',
                                      'state', 'artifacts')
                    r = {field: s.get(field) for field in SESSION_FIELDS}
                    return jsonify(
                        success=True,
                        data={'session': r, 'issue': issue, 'scan': j['scan']})
    else:
...
def api_scan_start():
    ...
    # Start the scan
    r = requests.put(
        config['backend-api']['url'] + '/scans/' + scan['id'] + '/control/',
...
def api_scan_stop():
    ...
    # Stop the scan
    r = requests.put(
        config['backend-api']['url'] + '/scans/' + scan_id + '/control/',
        headers={'Content-Type': 'text/plain'},
        data='STOP',
        params={'email': session['email']})
    r.raise_for_status()
...
def requires_ws_auth(*decorator_args):
    ...
    def decorator(view):
        ...
        def check_api_key(*args, **kwargs):
            ...
            if (len(user.get('api_key', '')) == 0
                    or user.get('api_key') != api_key):
                return jsonify(success=False, reason='invalid-api-auth')
            g.api_user = user
            return view(*args, **kwargs)
...
#  GET /ws/sites?url=$URL retrieve all sites that match url (only direct match
#                         now, returns 1 result)
#
...
def ws_get_scans():
    ...
    if limit:
        limit = int(limit)
    scanz = _backend_get_scans(request.args.get(
        'site_id'), request.args.get('plan_name'), limit)
    return Response(
        json.dumps({
            'success': True,
            'scans': scanz
...
def ws_get_scan(scan_id):
    # TODO , params={'email': session['email']})
    r = requests.get(config['backend-api']['url'] + '/scans/' + scan_id)
    if r.json()['success']:
...
def put_ws_scans():
    ...
    configuration = {'target': site['url'], 'callback': {
        'url': request.json['callbackURL'], 'events': ['scan-state']}}
